...
from . import reconstruction_metrics
from . import editing_metrics
...
def gather_results_across_processes(results):
...
def calc_metric(metric, **kwargs): # See metric_utils.MetricOptions for the full list of arguments.
    ...
    # Broadcast results.
    if metric=="im64_edit_full":
        if opts.num_gpus > 1:
            results = gather_results_across_processes(results)
...
@register_metric
def recon_full(opts):
    assert opts.test_dataset_kwargs
    opts.test_dataset_kwargs.update(max_size=None, xflip=False)
    metrics_list = ["mse","psnr","ssim","lpips"]
    metrics_dict = reconstruction_metrics.compute_recon_metrics(opts, metrics_list=metrics_list)
    return metrics_dict
...
@register_metric
def cifar10_edit_full(opts):
    assert opts.test_dataset_kwargs
    opts.test_dataset_kwargs.update(max_size=None, xflip=False)
    metrics_dict = editing_metrics.compute_editing_metrics(opts, "cifar10")
    return metrics_dict

@register_metric
def im64_edit_full(opts):
    assert opts.im64_subg_dir
    assert opts.im64_visualize_dir
    metrics_dict = editing_metrics.compute_editing_metrics_presampled_2k(opts)
    return metrics_dict
